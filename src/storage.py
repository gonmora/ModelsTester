...
def load_dataframe(name: str) -> pd.DataFrame:
    """Load a pandas DataFrame from disk with the given name."""
    path = _df_path(name)
...
def save_feature(df_name: str, feature_name: str, data: pd.DataFrame | pd.Series) -> None:
    ...
    _ensure_dir_exists(path)
    # If a Series is provided, convert it to a one-column DataFrame
    if isinstance(data, pd.Series):
        data = data.to_frame(name=feature_name)
    data.to_parquet(path)


def load_feature(df_name: str, feature_name: str) -> pd.DataFrame:
    """Load a feature DataFrame from disk."""
    path = _feature_path(df_name, feature_name)
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"Feature '{feature_name}' for DataFrame '{df_name}' not found at {path}"
        )
    return pd.read_parquet(path)


def feature_exists(df_name: str, feature_name: str) -> bool:
    """Return True if the given feature exists on disk."""
...
def save_target(df_name: str, target_name: str, data: pd.DataFrame | pd.Series) -> None:
    """Save a target DataFrame or Series to disk."""
    path = _target_path(df_name, target_name)
    _ensure_dir_exists(path)
    if isinstance(data, pd.Series):
        data = data.to_frame(name=target_name)
    data.to_parquet(path)


def load_target(df_name: str, target_name: str) -> pd.DataFrame:
    """Load a target DataFrame from disk."""
    path = _target_path(df_name, target_name)
    if not os.path.exists(path):
        raise FileNotFoundError(
            f"Target '{target_name}' for DataFrame '{df_name}' not found at {path}"
        )
    return pd.read_parquet(path)


def target_exists(df_name: str, target_name: str) -> bool:
    """Return True if the given target exists on disk."""
